...
function Invoice() {
  ...
  const paginatedData = invoice.slice(
  ...
  let deletedata = async (id) => {
    try {
      ...
      setInvoice((prevUsers) => prevUsers.filter((user) => user.id !== id));
    } catch (error) {
            ...
            <div className="d-flex justify-content-between mb-3">
              <div className="d-flex flex-row w-100">
                ...
                <div className="search-gallery">
                  <input
                    className="form-control py-4 mt-2 mt-lg-0 rounded-2 cart-cart ms-0 border"
                    placeholder="Search..."
                    type="search"
                    name="search"
                    value={search}
                  ...
                  {Array.isArray(paginatedData) && paginatedData.length > 0 ? (
                    paginatedData.map((data, key) => (
                      <tr key={key}>
                        ...
                        <td className="cart-cart">
                          <span className="sliders1 sales-font">
                            <Link
                              to={`/admin/ecommerce/invoices/edit/${data.id}`}
                            >
                              {data.order_number}
                        ...
                        <td className="cart-cart sales-font">
                          <Link
                            to={`/admin/ecommerce/invoices/edit/${data.id}`}
                          >
                        ...
                        <td>
                          <span className="fw-medium lh-base px-2 bulk1 sales-font">
                            ${data.total}
                          </span>
                        </td>
            ...
            {invoice.length > itemsPerPage && (
              <nav className="mt-3">
                <ul className="pagination justify-content-center">
                  ...
                  {Array.from({
                    length: Math.ceil(invoice.length / itemsPerPage),
                  }).map((_, index) => (
                    <li
                      key={index}
                      className={`page-item ${
                    ...
                    <button
                      ...
                      onClick={() =>
                        setCurrentPage((prev) =>
                          Math.min(
                            prev + 1,
                            Math.ceil(invoice.length / itemsPerPage)
                          )
                        )
                      }
                    >
