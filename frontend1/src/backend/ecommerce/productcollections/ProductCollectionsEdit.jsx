...
function ProductCollectionsEdit() {
  let { id } = useParams();
  ...
  let handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://89.116.170.231:1600/collectionupdate/${id}`,
        formData
      );
  ...
  let alldata = async () => {
    let response = await axios.get(
      `http://89.116.170.231:1600/collectionsome/${id}`
    );
      ...
      <nav className="breadcrumb-container text-center">
        <ol className="breadcrumb d-flex flex-wrap">
          ...
          <li className="breadcrumb-item fw-medium ms-2">
            <Link to="/admin/ecommerce/product-collections">
              PRODUCT COLLECTIONS
            </Link>
          </li>
...
export default ProductCollectionsEdit;
