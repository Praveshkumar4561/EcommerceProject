...
function ProductInventory() {
  ...
  const reloadInvent = async (id, updatedUserData) => {
    const updatedUser = updatedUserData.find((user) => user.id === id);
    if (!updatedUser || updatedUser.minimumorder === undefined) {
      console.error(`No minimumorder value found for id ${id}`);
      return;
    }
    const updatedData = {
      minimumorder: updatedUser.minimumorder,
    };
    try {
  ...
  return (
    <>
      <Helmet>
        ...
        <title>Product Inventory | RxLYTE</title>
      ...
      <nav className="breadcrumb-container text-center inventory-typo">
          ...
          <div className="card-body">
            <div className="d-flex justify-content-between mb-3 flex-row">
              <div className="d-flex">
                ...
                <input
                  className="form-control py-4 rounded-2 bulk mt-2 mt-lg-0 ms-0 border"
                  placeholder="Search..."
                  type="search"
                  name="search"
                  value={search}
