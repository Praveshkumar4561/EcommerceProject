...
function ProductOptions() {
  ...
  let searchbar = async () => {
    let response = await axios.get(`http://50.18.56.183:1600/searchproductoption/${search}`);
    setUser(response.data);
  };

  let alldata = async () => {
    let response = await axios.get("http://50.18.56.183:1600/productoptiondata");
    setUser(response.data);
  };

  let deletedata = async (id) => {
    await axios.delete(`http://50.18.56.183:1600/deleteproductoptions/${id}`, user);
  ...
  return (
    <>
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 table-announce w-auto d-flex justify-content-center align-items-center">
        <div class="card mt-3 testimonial">
          <div class="card-body">
            <div class="d-flex justify-content-between mb-3">
              <div class="d-flex flex-row">
                <div class="btn-group me-2">
                  <button
                    aria-expanded="false"
                    class="btn btn-secondary dropdown-toggle d-flex flex-row align-items-center py-4 btn-announ bulk"
                ...
                <button
                  class="btn btn-secondary me-2 py-4 d-flex btn-announ bulk mt-0 mt-lg-0"
                  type="button"
                >
                  Filters
                </button>
                <input
                  class="form-control py-4 rounded-2 bulk mt-0 mt-lg-0"
                  placeholder="Search..."
                  type="text"
                  name="search"
                  value={search}
              ...
              <div className="mt-2 mt-sm-0 mt-lg-0 d-flex ms-2 flex-row">
                <Link to="/admin/ecommerce/options/create">
                  <button
                    class="btn btn-create me-2 bulk d-flex flex-row align-items-center"
                    type="button"
                  >
                ...
                <button
                  class="btn btn-reload bulk border d-flex flex-row align-items-center"
                  type="button"
                >
                  ...
                  {Array.isArray(user) && user.length > 0 ? (
                    user.map((data, key) => (
                      <tr key={key}>
                        ...
                        <td>
                          <Link to={`/admin/ecommerce/options/edit/${data.id}`}>
                            {data.name}
                          </Link>
                        </td>
                        ...
                        <td>
                          <span
                            className="badge lh-base px-2 ms-4 status-blog fw-light"
                            style={{
                              backgroundColor:
                                data.featured === "No" ? "red" : "green",
                              color: "white",
                            }}
                          >
                            {data.featured}
                        ...
                        <td style={{ whiteSpace: "nowrap" }}>
                          <button class="btn btn-edit me-2" type="button">
                            <Link
                              to={`/admin/ecommerce/options/edit/${data.id}`}
                            >
                              <FontAwesomeIcon
                                icon={faPenToSquare}
