...
function ProductAttributesCreate() {
  ...
  const validateForm = () => {
    const newErrors = {};
    ["title", "slug", "sort", "status"].forEach((field) => {
      if (!user[field]?.toString().trim()) {
        newErrors[field] = `${
  ...
  const handleSubmit = async (e) => {
    ...
    const attributesToSend = attributes.map((attr) => ({
      title: attr.title,
      isDefault: attr.isDefault,
      color: attr.color,
      image: attr.file?.name || attr.image || "",
    }));
    ...
    attributes.forEach((attr) => {
      if (attr.file) {
        formData.append("attributeImages", attr.file);
      }
    });
    try {
  ...
  const addNewAttribute = (e) => {
    e.preventDefault();
    const newId =
      attributes.length > 0 ? attributes[attributes.length - 1].id + 1 : 1;
    setAttributes([
      ...attributes,
      {
        id: newId,
  ...
  return (
    <>
      <Helmet>
        ...
        <title>New product attributes | RxLYTE</title>
        ...
        <meta
          ...
          content="Copyright 2025 © RxLYTE. All rights reserved."
        />
        <meta property="og:title" content="New product attributes | RxLYTE" />
      ...
      <div className="container-fluid">
        <div className="container">
          <div className="row d-flex flex-row flex-xxl-nowrap flex-xl-nowrap gap-3 w-100 ms-md-1">
            <div className="col-12 col-lg-8 border rounded customer-page customer-page2 text-start">
              <form>
                  ...
                  <table className="table table-bordered">
                    <thead>
                      <tr>
                        ...
                        <th className="text-center fw-light">Value</th>
