...
function AdminDetails() {
  ...
  let serachbar = async () => {
    const response = await axios.get(
      `http://89.116.170.231:1600/adminsearch/${search}`
    );
    setUser(response.data);
  };

  let showdata = async () => {
    let response = await axios.get("http://89.116.170.231:1600/alladmindata");
    setUser(response.data);
  };
  ...
  let deletedata = async (id) => {
    try {
      await axios.delete(`http://89.116.170.231:1600/admindelete/${id}`);
    ...
    <>
      ...
      <div
        className={`sidebar-container
          ${isNavbarExpanded && isMobile ? "expanded" : ""}`}
      >
        <div className="sidebar-back-plat mt-2">
          <ul className="list-unstyled d-flex flex-column text-white ms-3">
            <li>
      ...
      <nav className="breadcrumb-container text-center">
        <ol className="breadcrumb d-flex flex-wrap">
          ...
          <li className="breadcrumb-item fw-medium text-dark">USERS</li>
        </ol>
      </nav>
                  ...
                  {Array.isArray(paginatedData) && paginatedData.length > 0 ? (
                    paginatedData.map((data, key) => (
                      <tr key={key}>
                        ...
                        <td>
                          <Link to={`/admin/system/users/profile/${data.id}`}>
                            {data.username}
                          </Link>
                        </td>
                        ...
                        <td>
                          <span className="badge-success rounded py-1 text-light">
                            Activated
                          </span>
                        </td>
