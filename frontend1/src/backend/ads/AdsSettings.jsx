...
function AdsSettings() {
  ...
  const handleFileUpload = (e) => {
    ...
    if (selectedFile) {
      if (
        ...
        setError(
          "The ads google adsense txt file must be a file of type: txt."
        );
        setFile(null);
      } else {
        setError("");
  ...
  const handleSaveSettings = async () => {
    if (!file) {
      toast.error("Please upload the .txt file.", {
    ...
    if (code.trim() === "" && clientId.trim() === "") {
      toast.error(
        "Please filled at least one: the Ads Snippet or the Client ID.",
        {
          position: "bottom-right",
          autoClose: 1000,
        }
    ...
    try {
      ...
      formData.append("googleAdsenseAutoAdsSnippet", code);
      formData.append("googleAdsenseUnitAdsClientID", clientId);
      formData.append("adsTxtFile", file);

      const response = await axios.post(
        "https://demo.webriefly.com/api/ads-settings",
        formData,
        {
      ...
      toast.success(
        response.data.message || "Ads settings saved successfully!",
        {
          position: "bottom-right",
          autoClose: 1000,
          ProgressBar: true,
  ...
  return (
    <>
      <Helmet>
        ...
        <title>Ads Settings | RxLYTE</title>
      ...
      <nav className="breadcrumb-container text-center">
        <ol className="breadcrumb ms-2 cart-cart d-flex flex-wrap flex-lg-nowrap">
          ...
          <li className="breadcrumb-item fw-medium text-dark">ADS SETTINGS</li>
        </ol>
      </nav>

      <div className="main-content container py-4 cart-cart d-flex flex-lg-row flex-column gap-3 ads-settings text-start">
        <div className="d-flex flex-column">
          <h4 className="mb-2">Ads Settings</h4>
          <span className="text-muted">Manage ads settings</span>
        </div>

        <div className="d-flex flex-column border px-2 rounded py-3 main-ads">
          <div className="mb-0 px-1 py-0">
            <label
              htmlFor="googleAdsenseAutoAdsSnippet"
            ...
            <p className="mb-0 mt-2">
              You can get this snippet from{" "}
              <Link
                to="https://adsense.google.com/start"
                target="_blank"
                rel="noreferrer"
                className="text-success ads-adsense"
              >
                Google AdSense
              </Link>
              , Go to <strong>Ads</strong> → <strong>Get code</strong> → Copy
              code snippet and paste here.
            </p>
          </div>

          <div className="mb-3">
            <label
              htmlFor="googleAdsenseUnitAdsClientID"
            ...
            <input
              type="text"
              className="form-control cart-cart py-4"
              id="googleAdsenseUnitAdsClientID"
              placeholder="ca-pub-123456789"
              value={clientId}
              onChange={(e) => setClientId(e.target.value)}
            />
            <p className="mb-0 mt-2 ms-1">
              You can get this snippet from{" "}
              <Link
                to="https://adsense.google.com/start"
                target="_blank"
              ...
              Go to <strong>Ads</strong> → <strong>Get code</strong> → Copy code
              snippet and paste here.
            </p>
          </div>

          <div className="mb-0 border rounded px-2 py-2 where-back main-ads1">
            <h6 className="mt-2 fw-bold">
              Where to get Google AdSense Client ID?
            </h6>
            <p className="mb-0">
              When you get the snippet code of unit ads, you can see this
              attribute <code>data-ad-client="ca-pub-123456789"</code>; the{" "}
              <code>ca-pub-123456789</code> is the unit ads client id.
            </p>
            <p className="mt-2">Google AdSense Unit Ads Snippet Example:</p>
            <pre className="p-2 adssense-unit text-light rounded">
...
<script>
    (adsbygoogle = window.adsbygoogle || []).push({});
          ...
          <div className="mt-3">
            <label htmlFor="googleAdsenseAdsTxt" className="form-label fw-bold">
              Your Google AdSense ads.txt
            </label>
            <input
              type="file"
              className="form-control pb-0 pt-1 ps-2 cart-cart"
              id="googleAdsenseAdsTxt"
          ...
          {!isFileSaved ? (
            <button
              type="button"
              className="btn btn-success d-flex cart-cart1 save-ads py-4 d-flex justify-content-center align-items-center mt-2"
            ...
            <div className="d-flex flex-column mt-2">
              <div className="d-flex flex-row flex-nowrap ms-1">
                View your ads.txt here:&nbsp;
                <Link
                  to={`/ads.txt`}
                  target="_blank"
                  className="mb-2 ads-adsense1"
                >
                  /ads.txt
                </Link>
              </div>
              <button
                type="button"
                className="btn btn-danger d-flex cart-cart1 delete-ads py-4 d-flex justify-content-center align-items-center ms-1"
