...
function AdsPage() {
  ...
  let searchbar = async () => {
    let response = await axios.get(
      `http://52.9.253.67:1600/adsearch/${search}`
    );
    setUser(response.data);
  };

  let alldata = async () => {
    let response = await axios.get("http://52.9.253.67:1600/adsdata");
    setUser(response.data);
  };

  let deletedata = async (id) => {
    await axios.delete(`http://52.9.253.67:1600/deleteads/${id}`, user);
    ...
    <>
      ...
      <nav className="breadcrumb-container text-center">
        <ol className="breadcrumb ms-2">
          ...
          <li className="breadcrumb-item fw-medium text-dark">ADS</li>
        </ol>
      </nav>
