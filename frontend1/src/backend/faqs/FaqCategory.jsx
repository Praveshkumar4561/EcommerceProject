...
function FaqCategory() {
  ...
  let searchbar = async () => {
    let response = await axios.get(
      `http://52.8.59.14:1600/faqsearchcategory/${search}`
    );
    setUser(response.data);
  };

  let alldata = async () => {
    let response = await axios.get("http://52.8.59.14:1600/faqcategorydata");
    setUser(response.data);
  };

  let deletedata = async (id) => {
    await axios.delete(`http://52.8.59.14:1600/faqcategorydelete/${id}`, user);
                  ...
                  {Array.isArray(user) && user.length > 0 ? (
                    user.map((data, key) => (
                      <tr key={key}>
                        ...
                        <td>
                          <span className="badge badge-success lh-base fw-light">
                            {data.status}
                          </span>
                        </td>
