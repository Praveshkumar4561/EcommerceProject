...
function CustomerAddress() {
  ...
  const alldata = async () => {
    try {
      let response = await axios.get(
        "https://demo.webriefly.com/api/userdashboarddata"
      );
  ...
  let removedata = async (id) => {
    await axios.delete(`https://demo.webriefly.com/api/deleteuser/${id}`, user);
    try {
  ...
  useEffect(() => {
    ...
    const filtered = customer.filter((customer) => {
      return customer.active === "yes" && new Date(customer.end_date) >= now;
    });
  ...
  return (
    <>
      <Helmet>
        <title>My Addresses - Manage Shipping & Billing Info | Rxlyte</title>
        <meta
          name="description"
      ...
      <div className="container-fluid">
        <div className="row align-items-start justify-content-between text-center mt-lg-0 mt-0 pt-0 pt-lg-0 bg-light ms-0">
            ...
            <div className="col-12 col-sm-12 col-md-12 col-lg-6 bg-body shadow-lg customer-dashboard1 text-start rounded-0 mb-2 ms-lg-2 ms-sm-2 border d-flex flex-column align-items-center py-5 overflow-hidden">
              {user.length === 0 ? (
                <>
                  ...
                  <span className="text-center mt-2 fs-4 button-orders">
                    No addresses!
                  </span>
                  <span className="text-center mt-1 fs-6 button-orders">
